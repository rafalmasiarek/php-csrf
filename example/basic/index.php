...
/**
 * DEMO ONLY:
 * For this basic test we store a 32-byte key in the PHP session,
...
$csrf = new Csrf($key, 900); // TTL = 900s
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Capture full debug BEFORE validate(), so we see exact state & payload
    $debug = $csrf->debugValidate($token);
    ...
    // Initial GET – generate token and inspect its internal state
    $token  = $csrf->generate();
    $debug  = $csrf->debugValidate($token);
    $status = 'fresh';
}
...
<head>
    <meta charset="utf-8">
    <title>CSRF Basic Debug Demo</title>
    <style>
        body {
...
<body>
    <div class="wrapper">
        ...
        <div class="panel">
            ...
            <?php else: ?>
                <h1 class="status-fresh">🔐 CSRF demo – fresh token generated</h1>
                <p class="msg">
                    Submit the form below to see CSRF validation in action.
                </p>
            <?php endif; ?>
